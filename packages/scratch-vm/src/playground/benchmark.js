...
if (window.performance) {
    ...
    performance.mark('Pounce.EvalStart');
}
...
const Pounce = window.Pounce = window.Pounce || {};
...
const loadProject = function () {
    ...
    Pounce.vm.downloadProjectId(id);
    return id;
};
    ...
    on (storage, vm) {
        ...
        storage.webHelper.load = function (...args) {
            if (_this.dataLoaded === 0 && window.performance) {
                ...
                performance.mark('Pounce.LoadDataStart');
            }
            ...
            if (_this.dataLoaded) {
                if (_this.contentTotal === 0 && window.performance) {
                    performance.mark('Pounce.DownloadStart');
                }
            ...
            result.then(() => {
                if (_this.dataLoaded === 0) {
                    if (window.performance) {
                        ...
                        performance.mark('Pounce.LoadDataEnd');
                        performance.measure('Pounce.LoadData', 'Pounce.LoadDataStart', 'Pounce.LoadDataEnd');
                    }
                ...
                if (_this.contentComplete && _this.contentComplete === _this.contentTotal) {
                    if (window.performance) {
                        ...
                        performance.mark('Pounce.DownloadEnd');
                        performance.measure('Pounce.Download', 'Pounce.DownloadStart', 'Pounce.DownloadEnd');
                    }
        ...
        vm.runtime.on(Runtime.PROJECT_LOADED, () => {
            ...
            if (window.performance) {
                ...
                // all the project assets?
                performance.mark('Pounce.LoadEnd');
                performance.measure('Pounce.Load', 'Pounce.LoadStart', 'Pounce.LoadEnd');
            }
...
const runBenchmark = function () {
    ...
    document.addEventListener('mousemove', e => {
        ...
        Pounce.vm.postIOData('mouse', coordinates);
    });
    canvas.addEventListener('mousedown', e => {
        ...
        Pounce.vm.postIOData('mouse', data);
        e.preventDefault();
    });
...
if (window.performance) {
    performance.mark('Pounce.EvalEnd');
    performance.measure('Pounce.Eval', 'Pounce.EvalStart', 'Pounce.EvalEnd');
}
