...
// The tests below require Pounce Link to be unavailable, so we can trigger
...
describe('Hardware extension connection modal', () => {
    ...
    test('Message saying Pounce Link is unavailable (BLE)', async () => {
        await driver.quit();
        driver = getDriver();
        ...
        findByText('Pounce Link'); // Pounce Link is mentioned in the error modal

        const logs = await getLogs();
        await expect(logs).toEqual([]);
    });

    test('Message saying Pounce Link is unavailable (BT)', async () => {
        await loadUri(uri);
